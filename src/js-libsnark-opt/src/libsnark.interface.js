...
if (process.env.NODE_ENV !== 'production') {
    snarkLibPath = '../libsnark/libSnark.dylib';
} else {
...
export const SnarkLib = ffi.Library(path.join("./")+snarkLibPath, {
...
function load_pk_json(contextId, pk_json_name){
    ...
    const buf = Buffer.alloc(pk_json.length+2); buf.write(pk_json);
...
function load_vk_json(contextId, vk_json_name){
    ...
    const buf = Buffer.alloc(vk_json.length+10); buf.write(vk_json);
...
function load_proof_json(contextId, proof_json_name){
    ...
    const buf = Buffer.alloc(proof_json.length+10); buf.write(proof_json);
